...
class GradAttneion:
    ...
    def __call__(self, input, index=None):
        ...
        for i in range(input.size(0)):
            self.attentions = []
            self.attention_gradients = []
            cls_logits_bi, attn_weights_bi = self.model(input[[i]])
            if index == None:
        ...
        cls_logits = torch.stack(cls_logits, dim=0) #BxC
        n = input.size(0)
        c = self.model.module.num_classes
        patch_size = self.model.module.patch_embed.patch_size
        h, w = input.shape[2]//patch_size[0], input.shape[3]//patch_size[1]
        attns = torch.stack(attns, dim=1)[:,:,:,0,1:] #LxBx6x196
        mean_attns = torch.mean(attns, dim=2) #LxBx196
        mean_attns = mean_attns.sum(0).reshape([n, h, w]).unsqueeze(1)  # Bx1x14x14
        attns_grad = torch.stack(attns_grad, dim=1)[:,:,:,0,1:] #LxBx6x196
        ...
        grad_attns = grad_attns.sum(0).reshape([n, h, w]).unsqueeze(1) #Bx1x14x14
        mean_attn_cams = mean_attns * cls_logits.unsqueeze(2).unsqueeze(3)
        grad_attn_cams = grad_attns * cls_logits.unsqueeze(2).unsqueeze(3)

        return cls_logits, mean_attn_cams, grad_attn_cams, mean_attns.expand(-1,c,-1,-1), grad_attns.expand(-1,c,-1,-1)
...
def creat_model(cfg, args):
    ...
    print(model)
    if args.resume and args.resume[-4:] == '.pth':
...
def main():
    ...
    if args.resume[-4:] == '.pth':
        ...
        cfg.BASIC.SAVE_DIR = os.path.join(args.resume[:args.resume.find('/ckpt')], 'eval', '{}{}-CAM-THR{}_BS{}'.
                                          format(cfg.MODEL.CAM_MERGE, cfg.MODEL.ATTN_DISCARD_RATIO, cfg.MODEL.CAM_THR, cfg.TEST.BATCH_SIZE))
...
def val_loc_one_epoch(val_loader, model, device, criterion, writer, cfg, update_val_step):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, bbox, image_names, one_hot_target) in enumerate(val_loader):
            ...
            cls_logits, attns = model(input)
            # cls_logits, mean_attn_cams, grad_attn_cams, mean_attns, grad_attns = model_gradwrap(input)
            n = input.size(0)
            c = model.module.num_classes
            patch_size = model.module.patch_embed.patch_size
            h, w = input.shape[2]//patch_size[0], input.shape[3]//patch_size[1]
            attns = torch.stack(attns, dim=0) #LxBx6x197x197
            mean_attns = torch.mean(attns, dim=2) #LxBx197x197
            mean_attns = mean_attns.sum(0)[:,0,1:].reshape([n, h, w]).unsqueeze(1)  # Bx1x14x14
            mean_attn_cams = mean_attns * cls_logits.unsqueeze(2).unsqueeze(3)
            if cfg.TEST.CAM_TYPE == 'MeanAttenCam':
                cams = mean_attn_cams
            elif cfg.TEST.CAM_TYPE == 'MeanAtten':
                cams = mean_attns.expand(-1,c,-1,-1)
