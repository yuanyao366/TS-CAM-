...
def creat_model(cfg, args):
    ...
    print('load pretrained ts-cam model:{}.'.format(args.resume))
    ...
    return device, model, optimizer, cls_criterion_pt, cls_criterion_ct, checkpoint['epoch']
...
def main():
    ...
    datetime_dir = 'Logit{}_Loss{}_{}_From_{}'.format(cfg.TEST.CLS_LOGITS, cfg.SOLVER.LOSS, cfg.BASIC.TIME, args.resume.split('/')[-3])
    if args.extra_token_blk_idx >= 0:
    ...
    device, model, optimizer, cls_criterion_pt, cls_criterion_ct, checkpoint_epoch = creat_model(cfg, args)
    ...
    update_train_step = 0
    update_val_step = 0
    for epoch in range(1+checkpoint_epoch, cfg.SOLVER.NUM_EPOCHS+1+checkpoint_epoch):
...
def train_one_epoch(train_loader, model, device, criterion_pt, criterion_ct, optimizer, epoch,
    ...
    for i, (input, target, one_hot_target) in enumerate(train_loader):
        ...
        if cfg.SOLVER.LOSS == 'BCE':
            loss_ct = criterion_ct(cls_logits_ct, one_hot_target) * 10
        else:
