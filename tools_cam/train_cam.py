...
def creat_model(cfg, args):
    ...
    print(model)
    if cfg.MODEL.ARCH == 'vgg16_cam':
        optimizer = torch.optim.SGD([{'params': model.features_params, 'lr_mult': 0.1},
                                     {'params': model.classifier_params, 'lr_mult': 1}],
                                    lr=cfg.SOLVER.START_LR,
                                    momentum=cfg.SOLVER.MUMENTUM,
...
def main():
    ...
    cfg.BASIC.SAVE_DIR = os.path.join('ckpt', cfg.DATA.DATASET, '{}_CAM-NORMAL_SEED{}_{}-CAM-THR{}_BS{}'.format(
        cfg.MODEL.ARCH, cfg.BASIC.SEED, cfg.MODEL.CAM_MERGE, cfg.MODEL.CAM_THR, cfg.TRAIN.BATCH_SIZE), cfg.BASIC.TIME)
...
def train_one_epoch(train_loader, model, device, criterion, optimizer, epoch,
    ...
    for i, (input, target, one_hot_target) in enumerate(train_loader):
        ...
        cls_logits = model(input, return_cam=False, label=target, cfg=cfg)
...
def val_loc_one_epoch(val_loader, model, device, criterion,epoch, writer, cfg, update_val_step):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, bbox, image_names, one_hot_target) in enumerate(val_loader):
            ...
            cls_logits, cams = model(input, return_cam=True, label=target, cfg=cfg)
            loss = criterion(cls_logits, target)
