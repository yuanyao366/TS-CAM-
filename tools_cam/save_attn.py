...
def creat_model(cfg, args):
    ...
    if cfg.MODEL.ARCH == 'vgg16_cam':
        model = vgg16_cam(pretrained=True, num_classes=cfg.DATA.NUM_CLASSES)
    elif 'deit' in cfg.MODEL.ARCH:
...
def main():
    ...
    update_val_step, _, cls_top1, cls_top5, loc_top1, loc_top5, loc_gt_known, wrong_details = \
        val_loc_one_epoch(val_loader, model, device, cls_criterion, cfg, update_val_step)
...
def val_loc_one_epoch(val_loader, model, device, criterion, cfg, update_val_step):
        ...
        for i, (input, target, bbox, image_names, one_hot_target) in enumerate(val_loader):
            ...
            cls_top1_b, cls_top5_b, loc_top1_b, loc_top5_b, loc_gt_known_b, top1_loc_right_b, \
                top1_loc_cls_b,top1_loc_mins_b, top1_loc_part_b, top1_loc_more_b, top1_loc_wrong_b = \
                    save_cls_loc(input, target, bbox, cls_logits, cams, image_names, cfg, 0, attns_map, feats_map, '{}_boxed_image'.format(cfg.TEST.CLS_LOGITS), model_name)
