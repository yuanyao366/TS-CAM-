...
class GradAttneion:
    ...
    def __call__(self, input, index=None):
        self.attentions = []
        self.attention_gradients = []
        cls_logits, cams, feats, attn_weights = self.model(input, return_cam=True, vis=True)
        if index == None:
            index = np.argmax(cls_logits.cpu().data.numpy(), axis=-1)
        one_hot = np.zeros((cls_logits.size()[0], cls_logits.size()[-1]), dtype=np.float32)
        one_hot[np.arange(one_hot.shape[0]), index] = 1
        one_hot = torch.from_numpy(one_hot).requires_grad_(True).to(input.get_device())
        one_hot = torch.sum(one_hot * cls_logits)
        self.model.zero_grad()
        one_hot.backward()
        n, c, h, w = feats.shape
        mean_attns = attn_weights.sum(0)[:, 0, 1:].reshape([n, h, w]).unsqueeze(1)
        attns = torch.stack(self.attentions)[:,:,:,0,1:] #LxBx6x196
        attns_grad = torch.stack(self.attention_gradients)[:,:,:,0,1:]
...
def val_loc_one_epoch(val_loader, model, device, criterion, writer, cfg, update_val_step):
    ...
    for i, (input, target, bbox, image_names, one_hot_target) in enumerate(val_loader):
        ...
        if cfg.TEST.CAM_TYPE == 'MeanAttenCam':
            cams = mean_attn_cams
        elif cfg.TEST.CAM_TYPE == 'BatchGradinAttenCam' or cfg.TEST.CAM_TYPE == 'BatchGradinMaskAttenCam':
            cams = grad_attn_cams
        elif cfg.TEST.CAM_TYPE == 'Feat':
            cams = feats
        elif cfg.TEST.CAM_TYPE == 'MeanAtten':
            cams = mean_attns
        elif cfg.TEST.CAM_TYPE == 'BatchGradinAtten' or cfg.TEST.CAM_TYPE == 'BatchGradinMaskAtten':
            cams = grad_attns
