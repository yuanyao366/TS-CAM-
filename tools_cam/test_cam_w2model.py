...
def main():
    ...
    log_file = os.path.join(cfg.BASIC.SAVE_DIR, 'Log_{}_'.format(cfg.TEST.CLS_LOGITS) + cfg.BASIC.TIME + '.txt')
...
def val_loc_one_epoch(val_loader, model_cls, model_loc, device, criterion, cfg, update_val_step):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, bbox, image_names, one_hot_target) in enumerate(val_loader):
            ...
            cls_logits_pt, cams = model_loc(input, return_cam=True)
            loss = criterion(cls_logits_pt, target)
            if cfg.TEST.CLS_LOGITS == 'PT':
