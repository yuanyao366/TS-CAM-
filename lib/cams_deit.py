...
def blend_cam_nobox(image, cam, alpha=0.5):
    cam = (cam * 255.).astype(np.uint8)
    heatmap = cv2.applyColorMap(cam, cv2.COLORMAP_JET)
...
def evaluate_cls_loc(input, cls_label, bbox_label, logits, cams, image_names, cfg, epoch, iou_th=0.5):
    """
    ...
    for b in range(batch):
        ...
        topk_cls, topk_loc, wrong_details=cls_loc_err(top_bboxes, cls_label[b], gt_bbox, topk=(1,5), iou_th=iou_th)
...
def save_cls_loc(input, cls_label, bbox_label, logits, cams, image_names, cfg, epoch, attns, feats, exp_name, model_name):
    """
    :param input: input tensors of the model
    :param cls_label: class label
    :param bbox_label: bounding box label
    ...
    _, top5_idx = logits.topk(5, 1, True, True)
    top5_idx = top5_idx.tolist()
    _, last5_idx = logits.topk(5, 1, False, True)
    last5_idx = last5_idx.tolist()
    ...
    batch = cams.shape[0]
    image = tensor2image(input.clone().detach().cpu(), cfg.DATA.IMAGE_MEAN, cfg.DATA.IMAGE_STD)
    save_dir = os.path.join(cfg.BASIC.SAVE_DIR, exp_name)
    mkdir(save_dir)
    for b in range(batch):
        gt_bbox = bbox_label[b].strip().split(' ')
        gt_bbox = list(map(float, gt_bbox))
        ...
        for cls_i in range(len(top5_idx[b])):
        # for cls_i in range(len(top1_idx[b])):
            cam_i = cams[b, [top5_idx[b][cls_i]], :, :]
            ...
            cam_blend_i = blend_cam_nobox(image[b], cam_i, 0.6)[0]
            sub_save_dir = os.path.join(save_dir, 'cams', 'top{}'.format(cls_i+1))
            mkdir(sub_save_dir)
            save_path = os.path.join(sub_save_dir, image_names[b].split('/')[-1].split('.')[0]+'_{}_cam_top{}_cls{}.jpg'.format(model_name, cls_i+1, top5_idx[b][cls_i]))
            cv2.imwrite(save_path, cam_blend_i)
            
            cam_i = cams[b, [last5_idx[b][cls_i]], :, :]
            cam_i = torch.mean(cam_i, dim=0, keepdim=True)
            cam_i = cam_i.detach().cpu().numpy().transpose(1, 2, 0)
            cam_i = resize_cam(cam_i, size=(cfg.DATA.CROP_SIZE, cfg.DATA.CROP_SIZE))
            cam_blend_i = blend_cam_nobox(image[b], cam_i, 0.6)[0]
            sub_save_dir = os.path.join(save_dir, 'cams', 'last{}'.format(cls_i+1))
            mkdir(sub_save_dir)
            save_path = os.path.join(sub_save_dir, image_names[b].split('/')[-1].split('.')[0]+'_{}_cam_last{}_cls{}.jpg'.format(model_name, cls_i+1, last5_idx[b][cls_i]))
            cv2.imwrite(save_path, cam_blend_i)

            if feats is not None:
                feat_i = feats[b, [top5_idx[b][cls_i]], :, :]
                feat_i = torch.mean(feat_i, dim=0, keepdim=True)
                feat_i = feat_i.detach().cpu().numpy().transpose(1, 2, 0)
                feat_i = resize_cam(feat_i, size=(cfg.DATA.CROP_SIZE, cfg.DATA.CROP_SIZE))
                feat_blend_i = blend_cam_nobox(image[b], feat_i, 0.6)[0]
                sub_save_dir = os.path.join(save_dir, 'feats', 'top{}'.format(cls_i + 1))
                mkdir(sub_save_dir)
                save_path = os.path.join(sub_save_dir, image_names[b].split('/')[-1].split('.')[0]+'_{}_feat_top{}_cls{}.jpg'.format(model_name, cls_i+1, top5_idx[b][cls_i]))
                cv2.imwrite(save_path, feat_blend_i)
                
                feat_i = feats[b, [last5_idx[b][cls_i]], :, :]
                feat_i = torch.mean(feat_i, dim=0, keepdim=True)
                feat_i = feat_i.detach().cpu().numpy().transpose(1, 2, 0)
                feat_i = resize_cam(feat_i, size=(cfg.DATA.CROP_SIZE, cfg.DATA.CROP_SIZE))
                feat_blend_i = blend_cam_nobox(image[b], feat_i, 0.6)[0]
                sub_save_dir = os.path.join(save_dir, 'feats', 'last{}'.format(cls_i + 1))
                mkdir(sub_save_dir)
                save_path = os.path.join(sub_save_dir, image_names[b].split('/')[-1].split('.')[0]+'_{}_feat_last{}_cls{}.jpg'.format(model_name, cls_i+1, last5_idx[b][cls_i]))
                cv2.imwrite(save_path, feat_blend_i)

            if attns is not None:
                attn_i = attns[b, [top5_idx[b][cls_i]], :, :]
                attn_i = torch.mean(attn_i, dim=0, keepdim=True)
                attn_i = attn_i.detach().cpu().numpy().transpose(1, 2, 0)
                attn_i = resize_cam(attn_i, size=(cfg.DATA.CROP_SIZE, cfg.DATA.CROP_SIZE))
                attn_blend_i = blend_cam_nobox(image[b], attn_i, 0.6)[0]
                sub_save_dir = os.path.join(save_dir, 'attns', 'top{}'.format(cls_i + 1))
                mkdir(sub_save_dir)
                save_path = os.path.join(sub_save_dir, image_names[b].split('/')[-1].split('.')[0]+'_{}_attn_top{}_cls{}.jpg'.format(model_name, cls_i+1, top5_idx[b][cls_i]))
                cv2.imwrite(save_path, attn_blend_i)
                
                attn_i = attns[b, [last5_idx[b][cls_i]], :, :]
                attn_i = torch.mean(attn_i, dim=0, keepdim=True)
                attn_i = attn_i.detach().cpu().numpy().transpose(1, 2, 0)
                attn_i = resize_cam(attn_i, size=(cfg.DATA.CROP_SIZE, cfg.DATA.CROP_SIZE))
                attn_blend_i = blend_cam_nobox(image[b], attn_i, 0.6)[0]
                sub_save_dir = os.path.join(save_dir, 'attns', 'last{}'.format(cls_i + 1))
                mkdir(sub_save_dir)
                save_path = os.path.join(sub_save_dir, image_names[b].split('/')[-1].split('.')[0]+'_{}_attn_last{}_cls{}.jpg'.format(model_name, cls_i+1, last5_idx[b][cls_i]))
                cv2.imwrite(save_path, attn_blend_i)
        ...
        # Get blended image
        # blend, heatmap = blend_cam(image[b], cam_b, estimated_bbox)
        blend, heatmap = blend_cam_nobox(image[b], cam_b, 0.6)
        # Get boxed image
        gt_score = cls_scores[b][top1_idx[b][0]]  # score of gt class
        boxed_image = draw_bbox(blend, iou, np.array(gt_bbox).reshape(-1,4).astype(np.int), estimated_bbox, gt_score, False)
        #sub_save_dir = os.path.join(save_dir, 'bboxes')
        #mkdir(sub_save_dir)
        #save_path = os.path.join(sub_save_dir, image_names[b].split('/')[-1].split('.')[0] + '_{}_bbox.jpg'.format(model_name))
        #cv2.imwrite(save_path, boxed_image)
