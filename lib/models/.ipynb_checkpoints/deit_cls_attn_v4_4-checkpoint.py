...
__all__ = [
    'deit_cls_attn_v4_4_tiny_patch16_224', 'deit_cls_attn_v4_4_small_patch16_224', 'deit_cls_attn_v4_4_base_patch16_224',
]
...
class TSAttn_V4_4(VisionTransformer):
    ...
    def forward(self, x, return_cam=False, vis=False):
        ...
        x_logits = self.head(x_cls).squeeze(-1)
        ...
        if self.training:
            ...
            cls_attns = attn_weights.sum(0)[:, new_num_patches+1:, 1:new_num_patches+1].reshape([n, self.num_cls_attn, P_H, P_W])
            ct0_attns = attn_weights.sum(0)[:, 0, 1:new_num_patches+1].reshape([n, P_H, P_W]).unsqueeze(1)
            cams = cls_attns                         # B * C * 14 * 14
            # cams = cls_attns
            # cams = feature_map
            ...
            if vis:
                return x_logits, cams, cls_attns, attn_weights

            return x_logits, cams#, feature_map, ct0_attns.expand(-1,self.num_classes,-1,-1), cls_attns
...
@register_model
def deit_cls_attn_v4_4_tiny_patch16_224(pretrained=False, **kwargs):
    model = TSAttn_V4_4(
...
@register_model
def deit_cls_attn_v4_4_small_patch16_224(pretrained=False, **kwargs):
    model = TSAttn_V4_4(
...
@register_model
def deit_cls_attn_v4_4_base_patch16_224(pretrained=False, **kwargs):
    model = TSAttn_V4_4(
