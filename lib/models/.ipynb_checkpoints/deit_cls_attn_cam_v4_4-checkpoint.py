...
__all__ = [
    'deit_cls_attn_cam_v4_4_tiny_patch16_224', 'deit_cls_attn_cam_v4_4_small_patch16_224', 'deit_cls_attn_cam_v4_4_base_patch16_224',
]
...
class Attention_ExtraTokenDecoder(Attention):
    ...
    def forward(self, x):
        ...
        attn_extra = (q_extra @ k_orig.detach().transpose(-2, -1)) * self.scale
        attn_extra = attn_extra.softmax(dim=-1)
        # weights = attn if self.vis else None
        weights_extra = attn_extra
        attn_extra = self.attn_drop(attn_extra)
        x_extra = (attn_extra @ v_orig.detach()).transpose(1, 2).reshape(B, self.num_extra_token, C)
...
class TSAttnCAM_V4_4(VisionTransformer):
...
#         nn.init.uniform_(self.extra_cls_token, -bound, bound)
#         trunc_normal_(self.extra_cls_token, std=.02)
    ...
    def forward(self, x, return_cam=False, vis=False, extra_token_block_idx=0):
        ...
        if self.training:
            ...
            attn_weights = torch.stack(attn_weights[extra_token_block_idx:])        # 12 * B * H * N * N
            ...
            # cams = cls_attns
            # cams = feature_map
            # cams = ct0_attns.expand(-1,self.num_classes,-1,-1)
            # cams = cls_attns * feature_map * ct0_attns
            # cams = (cls_attns+ct0_attns) * feature_map
            # cams = ct0_attns * feature_map
            # cams = cls_attns * ct0_attns

            if vis:
                all_cls_attns = attn_weights[:,:,new_num_patches+1:, 1:new_num_patches+1].reshape([attn_weights.size(0), n, self.num_cls_attn, h, w])
                # return x_logits_pt, x_logits_ct, cams, feature_map, attn_weights
                return {'logits': x_logits_pt, 'cams': cams, 'cls_attns': cls_attns, 'feats': feature_map, 'ct0_attns':ct0_attns.expand(-1,self.num_classes,-1,-1), 'all_cls_attns': all_cls_attns}

            return x_logits_pt, x_logits_ct, cams#, feature_map, ct0_attns.expand(-1,self.num_classes,-1,-1), cls_attns
...
@register_model
def deit_cls_attn_cam_v4_4_tiny_patch16_224(pretrained=False, **kwargs):
    model = TSAttnCAM_V4_4(
...
@register_model
def deit_cls_attn_cam_v4_4_small_patch16_224(pretrained=False, **kwargs):
    model = TSAttnCAM_V4_4(
    ...
    if pretrained:
        ...
        # if model.num_cls_attn == 200:
        #     checkpoint_add = torch.load('./ckpt/CUB/extra_cls_token.pth', map_location="cpu")
        #     checkpoint.update(checkpoint_add)
        # elif model.num_cls_attn == 1000:
        #     checkpoint_add = torch.load('./ckpt/ImageNet/extra_cls_token.pth', map_location="cpu")
        #     checkpoint.update(checkpoint_add)
...
@register_model
def deit_cls_attn_cam_v4_4_base_patch16_224(pretrained=False, **kwargs):
    model = TSAttnCAM_V4_4(
