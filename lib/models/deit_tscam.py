...
class TSCAM(VisionTransformer):
    ...
    def forward(self, x, label=None, return_cam=False, vis=False, cfg=None):
        ...
        if self.training:
            ...
            if cfg.TEST.CAM_TYPE == 'Ct0Attn':
                cams = ct0_attns.expand(-1,self.num_classes,-1,-1)
            elif cfg.TEST.CAM_TYPE == 'Feat':
                cams = feature_map
            else:
                cams = ct0_attns * feature_map                           # B * C * 14 * 14
            # cams = cams.expand(-1,self.num_classes,-1,-1)
            # cams = feature_map

            if vis:
                all_cls_attns = attn_weights[:,:, 0, 1:].reshape([attn_weights.size(0), n, h, w])
...
#                 return {'logits': x_logits, 'cams': cams, 'attns': ct0_attns.expand(-1, self.num_classes, -1, -1), 'feats': feature_map}
                return {'logits': x_logits, 'cams': cams, 'ct0_attns': ct0_attns.expand(-1, self.num_classes, -1, -1), 'feats': feature_map, 'all_cls_attns': all_cls_attns}
